...
namespace argparse
{
    ...
    template <typename T>
    std::string to_lower(const T &str_)
    { // both std::string and std::basic_string_view<char> (for magic_enum) are using to_lower
    ...
    template <>
    inline std::string get(const std::string &v) { return v; }
    template <>
    inline char get(const std::string &v) { return v.empty() ? throw std::invalid_argument("empty string") : v.size() > 1 ? v.substr(0, 2) == "0x" ? (char)std::stoul(v, nullptr, 16) : (char)std::stoi(v)
                                                                                                                          : v[0]; }
    template <>
    inline int get(const std::string &v) { return std::stoi(v); }
    template <>
    inline short get(const std::string &v) { return std::stoi(v); }
    template <>
    inline long get(const std::string &v) { return std::stol(v); }
    template <>
    inline bool get(const std::string &v) { return to_lower(v) == "true" || v == "1"; }
    template <>
    inline float get(const std::string &v) { return std::stof(v); }
    template <>
    inline double get(const std::string &v) { return std::stod(v); }
    template <>
    inline unsigned char get(const std::string &v) { return get<char>(v); }
    template <>
    inline unsigned int get(const std::string &v) { return std::stoul(v); }
    template <>
    inline unsigned short get(const std::string &v) { return std::stoul(v); }
    template <>
    inline unsigned long get(const std::string &v) { return std::stoul(v); }
    ...
    template <typename T>
    struct ConvertType : public ConvertBase
    {
        T data;
    ...
    struct Entry
    {
        ...
        Entry(ARG_TYPE type, const std::string &key, std::string help, std::optional<std::string> implicit_value = std::nullopt) : type(type),
                                                                                                                                   keys_(split(key)),
                                                                                                                                   help(std::move(help)),
        ...
        template <typename T>
        Entry &set_default(const T &default_value)
        {
        ...
        template <typename T>
        operator T() {} // When you get here  because you received an error, make sure all parameters of argparse are references (e.g. with `&`)
    ...
    private:
        ...
        void _apply_default()
        {
            ...
            else if (default_str_.has_value())
            { // in cases where a string is provided to the `set_default` function
    ...
    public:
        ...
        virtual void help()
        {
            ...
            for (const auto &[subcommand, subentry] : subcommand_entries)
            {
                cout << endl
                     << endl
                     << bold("Subcommand: ") << bold(subcommand) << endl;
        ...
        void parse(int argc, const char *const *argv, const bool &raise_on_error)
        {
            ...
            auto parse_param = [&](size_t &i, const std::string &key, const bool is_short, const std::optional<std::string> &equal_value = std::nullopt)
            {
                ...
                if (itt != kwarg_entries.end())
                {
                    ...
                    { // short values are not allowed to look ahead for the next parameter
                        if (is_value(i + 1))
                        {
            ...
            size_t arg_i = 0;
            for (; arg_i < arg_entries.size() && !arg_entries[arg_i]->_is_multi_argument; arg_i++)
            { // iterate over positional arguments until a multi-argument is found
            ...
            { // iterate from back to front, to ensure non-multi-arguments in the front and back are given preference
    ...
    template <typename T>
    T parse(int argc, const char *const *argv, const bool &raise_on_error = false)
    {
        T args = T();
