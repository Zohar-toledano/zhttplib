...
public:
	...
	int getTotalSize()
	{
		return sizeof(SharedQueue<T>) + maxSize * sizeof(T);
	}

	inline T * getStartOfArr(SharedQueue<T>** start)
	{
		return reinterpret_cast<T *>(*start + 1); // Point to the memory right after the struct
	}

	bool push(T &value,SharedQueue<T>** start,HANDLE* hMutex)
	{
		// rear = push_counter
	...
	bool pop(T &value,SharedQueue<T>** start,HANDLE* hMutex)
	{
		DWORD dwWaitResult = WaitForSingleObject(*hMutex, INFINITE);
		if (dwWaitResult != WAIT_OBJECT_0)
		{
	...
	static bool create(int maxSize, const char *name, SharedQueue<T> **res, HANDLE *hMapFile)
	{
		int totalSize = SharedQueue<T>::getTotalSize(maxSize);
	...
	static bool connect(int maxSize, const char *name, SharedQueue<T> **res, HANDLE *hMapFile)
	{
	...
	static bool getMemoryPointer(int maxSize, const char *name, SharedQueue<T> **res, HANDLE *hMapFile)
	{
		int totalSize = SharedQueue<T>::getTotalSize(maxSize);
		*res = reinterpret_cast<SharedQueue<T> *>(MapViewOfFile(
...
private:
	...
	HANDLE hMutex;
public:
	SharedQueueWrapper(int maxSize, const char *name, bool create = false) : creator(create)
	{
		if (create)
			SharedQueue<T>::create(maxSize, name, &queue, &hMapFile);

		else
			SharedQueue<T>::connect(maxSize, name, &queue, &hMapFile);
